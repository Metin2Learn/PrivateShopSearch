...
Add it UPPER:
InsertInMysql(owner, item, pTable[i]->display_pos, pTable[i]->price);
...
Add it UPPER:
	RemoveAllItemsFromMysql(ch->GetPlayerID());


---
Add in the bottom of the page:

void CShopManager::InsertInMysql(LPCHARACTER lpSeller, LPITEM item, BYTE bPos, DWORD dwItemPrice)
{
